...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    // Linha removida/modificada: Não estamos mais esperando o campo 'nome' do formulário
    $login = trim($_POST['login'] ?? '');
    $email = trim($_POST['email'] ?? '');
    $senha = $_POST['senha'] ?? '';
    // $confirmar = $_POST['confirmar'] ?? ''; // Removido na correção anterior
    ...
    if ($res->num_rows > 0) {
        ...
        // CORREÇÃO SQL: removido o campo 'nome' do INSERT, 
        // pois ele não está no formulário. Se você precisar do 'nome', 
        // adicione o input ao formulário e volte a incluir o 'nome' aqui.
        $sql2 = "INSERT INTO usuarios (login, email, senha) VALUES (?, ?, ?)";
        $stmt2 = $conn->prepare($sql2);
        
        // bind_param ajustado para 3 parâmetros (login, email, senha)
        $stmt2->bind_param("sss", $login, $email, $hash); 
