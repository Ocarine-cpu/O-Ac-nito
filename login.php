...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($result && $result->num_rows > 0) {
    ...
    if (password_verify($senha, $user['senha'])) {
      // campos de sessão para compatibilidade com o seu código existente
      $_SESSION['usuario_id'] = $user['id'];
      // agora colocamos o nome se existir, senão o login
      $_SESSION['usuario_nome'] = !empty($user['nome']) ? $user['nome'] : $user['login'];
